...
import { Cloud, CloudRain, Sun, Wind, Droplets, Gauge, MapPin } from "lucide-react"
...
export default function WeatherPage() {
  ...
  const handleMapClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const latRange = 10 // degrees
    const lonRange = 15 // degrees
    const centerLat = 31.1471
    const centerLon = 75.3412

    const newLat = centerLat + (y / rect.height - 0.5) * latRange
    const newLon = centerLon + (x / rect.width - 0.5) * lonRange

    setLatitude(newLat.toFixed(4))
    setLongitude(newLon.toFixed(4))
    setSubmitted({ lat: newLat, lon: newLon })
  }
    ...
    <main className="min-h-[calc(100vh-4rem)] bg-background">
      <section className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="max-w-3xl mx-auto space-y-6">
            ...
            <CardContent className="space-y-6">
              <div className="space-y-3">
                ...
                <div
                  ...
                  className="w-full h-64 bg-gradient-to-br from-blue-100 to-green-100 dark:from-blue-900 dark:to-green-900 rounded-lg border-2 border-dashed border-primary/50 cursor-crosshair flex items-center justify-center relative overflow-hidden"
                >
                  ...
                  <div className="relative z-10 text-center">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Click to select location</p>
                  </div>
                </div>
              </div>
          ...
          {/* Spline 3D model iframe inserted here */}
          <div className="w-full h-[400px] rounded-lg overflow-hidden border border-border">
            <iframe
              src="https://my.spline.design/hanastarterfilecopy-j2KTonRAbeigDgJ925vVLUCy-oZW/"
              frameBorder="0"
              width="100%"
              height="100%"
            ></iframe>
          ...
          {submitted && (
            <Card className="border border-border bg-card">
              ...
              <p className="text-destructive text-sm">
                      Couldn{"'"}t fetch weather. Please verify the coordinates.
                    </p>
                    )}
                {current && (
                    ...
                    <div className="rounded-lg bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 p-4">
                      ...
                      <ul className="text-sm text-green-800 dark:text-green-200 space-y-1">
                        ...
                        {current.windspeed > 20 && (
                            <li>• Strong winds ({current.windspeed} km/h) - Avoid spraying</li>
                          )}
                        {current.temperature > 35 && <li>• High temperature - Increase irrigation frequency</li>}
                        {current.temperature < 5 && (
                          <li>• Low temperature - Risk of frost damage, protect seedlings</li>
                        )}
